...
export function resolveLcmConfig(
  ...
  return {
    ...
    freshTailCount:
      ...
      32,
    leafMinFanout:
      (env.LCM_LEAF_MIN_FANOUT !== undefined
    ...
    largeFileSummaryProvider:
      env.LCM_LARGE_FILE_SUMMARY_PROVIDER?.trim() ??
      toStr(pc.largeFileSummaryProvider) ??
      "",
    largeFileSummaryModel:
      env.LCM_LARGE_FILE_SUMMARY_MODEL?.trim() ??
      toStr(pc.largeFileSummaryModel) ??
      "",
    expansionProvider:
