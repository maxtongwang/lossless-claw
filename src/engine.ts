...
import type { IConversationStore, ISummaryStore } from "./store/types.js";
import {
  blockFromPart,
  contentFromParts,
  ContextAssembler,
...
export class LcmContextEngine implements ContextEngine {
  ...
  /**
   * Create an LCM context engine.
   *
   * Two modes:
   * 1. SQLite (original): pass `database: DatabaseSync` — stores created internally
   * 2. Custom stores (DI): pass `stores: { conversationStore, summaryStore }` — host app provides persistence
   *
   * Custom stores mode skips SQLite connection, migration, and FTS5 detection entirely.
   */
  constructor(
    ...
    // Custom stores path — host app manages persistence (Postgres, Supabase, etc.)
    const isCustomStores =
      databaseOrStores &&
      typeof databaseOrStores === "object" &&
      "conversationStore" in databaseOrStores;

    if (isCustomStores) {
      this.db = null;
      this.fts5Available = false; // not applicable — host store handles FTS
      this.migrated = true; // host app manages schema
      // Cast: IConversationStore/ISummaryStore are supersets of the methods
      // actually called by engine/assembler/compaction/retrieval.
      this.conversationStore =
        databaseOrStores.conversationStore as unknown as ConversationStore;
      this.summaryStore =
        databaseOrStores.summaryStore as unknown as SummaryStore;
      this.compactionTelemetryStore =
        null as unknown as CompactionTelemetryStore;
      this.compactionMaintenanceStore =
        null as unknown as CompactionMaintenanceStore;
      this.info = {
        id: "lossless-claw",
      ...
      const database = databaseOrStores as DatabaseSync;
      this.db = database;
  ...
  private async rewriteTranscriptForRotate(params: {
    ...
    const linearizedEntries = entriesToKeep.map((entry) => {
      ...
      const entryWithId = nextEntry as Record<string, unknown>;
      previousEntryId =
        typeof entryWithId.id === "string" ? entryWithId.id : previousEntryId;
      return nextEntry;
    });
  ...
  async rotateSessionStorageWhileHoldingDatabaseLock(params: {
    ...
    if (!this.db) {
      return {
        kind: "unavailable",
        reason:
          "Rotate is not supported with custom stores — host app manages storage lifecycle.",
      };
    }
    const db = this.db;
    if (db.isTransaction) {
        ...
        try {
          return await withExclusiveDatabaseLock(
            ...
            async () => {
              ...
              try {
                backupPath = createLcmDatabaseBackup(db, {
